/* seal.tsx — the traveling 法印 seal + accumulating magic rings.
   Ported from app.jsx. The seal spring-tracks per-section [data-seal-anchor]
   elements so it scrolls WITH the content and hands off between sections. */

import { useEffect, useRef, useState } from 'react';
import { motion, AnimatePresence, useMotionValue, useSpring } from 'framer-motion';
import { MagicCircleSVG, RingPolySVG, type RingPolyKind } from './magic-circle';

// ─── MagicRings ───────────────────────────────────────────────────────────────
// Concentric 法印 rings that accumulate section-by-section into a full magic seal.
type Ring = {
  id: number; minSec: number; d: number; color: string; dash: boolean;
  tilt: number; prec: string; spin: string | null;
  poly?: RingPolyKind; deco?: 'ticks' | 'bagua' | 'gems';
};
...
// ─── TravelingSeal ────────────────────────────────────────────────────────────
type SealMode = 'floating' | 'stamped' | 'terminal' | 'moon' | 'complete' | 'condensed';
const SEAL_CONFIGS: { scale: number; mode: SealMode; char: string }[] = [
  { scale: 1,    mode: 'floating', char: '令' },  // 0 Hero: above the title
  { scale: 0.62, mode: 'stamped',  char: '道' },  // 1 What It Is: text is LEFT → seal RIGHT
  { scale: 0.5,  mode: 'terminal', char: '功' },  // 2 Features: heading top-left → seal top-right
  { scale: 0.52, mode: 'terminal', char: '示' },  // 3 Demo: terminal centered → seal right
  { scale: 0.78, mode: 'moon',     char: '境' },  // 4 CTA: moon above the heading
  { scale: 1.05, mode: 'complete', char: '令' },  // 5 Footer: lands on the reserved slot
];
...
export function TravelingSeal({ reduced }: { reduced: boolean }) {
  ...
  // Glue the seal to the active section's anchor every frame — it scrolls with
  // the page; the springs turn anchor hand-offs into the travel animation.
  useEffect(() => {
    if (reduced) return;
    let raf: number, el: Element | null = null;
    const track = () => {
