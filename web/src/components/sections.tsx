/* sections.tsx — page sections, in document order. */
...
// ─── FromDepth — element arrives from the back plane to the front ───────────
export function HeroSection({ settled }: { settled?: boolean }) {
  ...
  return (
    <section data-seal-section="0" style={{
      ...
      {/* Grand array behind the seal — `settled` skips the entrance so the intro's
          morphing copy lands on an already-formed twin (seamless handoff) */}
      <ArrayBackdrop variant="grand" size={680} opacity={.08} spin="ringCW 180s linear infinite"
        settled={settled} mark style={{ left:'50%', top:'calc(50% - 150px)' }} />

      {/* Heading — arrives from depth; glyphs rise with traveling gold glint */}
      <FromDepth delay={.2}>
      ...
      {/* Chinese subtitle */}
      <FromDepth delay={.85}>
        <p style={{ fontFamily:"'ZCOOL XiaoWei',serif", fontSize:'clamp(.95rem,2.2vw,1.35rem)',
      ...
      {/* Description */}
      <FromDepth delay={1.05}>
        <p style={{ fontFamily:"'EB Garamond',serif", fontSize:'clamp(1.05rem,1.9vw,1.3rem)',
          color:'#e9e4d6', opacity:.75, maxWidth:'460px', lineHeight:1.75,
          marginBottom:'2.8rem', textWrap:'pretty' }}>
          Summon any series from the shell —<br />
          no browser, no paywall, no tribute.
        </p>
      </FromDepth>

      {/* CTA group */}
      <FromDepth delay={1.25}
...
export function FeaturesSection() {
    return (
    <section data-seal-section="2" id="capabilities" aria-label="Features"
...
export function TerminalDemoSection({ compact }: { compact?: boolean }) {
  ...
  return (
    <section data-seal-section="3" id="demo" ref={sectionRef} aria-label="Terminal demo"
      ...
      <div style={{ width:'100%', maxWidth:'720px' }}>
        ...
        <motion.div style={{ rotateX: compact ? 0 : termRotX, scale:termScale, opacity:termOpac, y:termY,
          transformPerspective:1100, transformStyle:'preserve-3d' }}>
        <motion.div
          onMouseMove={e=>{ if (compact) return; const r=e.currentTarget.getBoundingClientRect();
          ...
          <div style={{ background:'#0c1410', padding:'1.4rem 1.6rem',
            ...
            lineHeight:1.75, minHeight: compact ? '250px' : '320px' }}>
...
export function CTASection() {
  ...
  return (
    <section data-seal-section="6" id="enter" aria-label="Install CTA"
      ...
      <div data-seal-anchor="6" aria-hidden="true" style={{
      ...
      {/* Stars / ember effect via box-shadow trick */}
      <div aria-hidden="true" style={{
...
export function FooterMagicSeal() {
  ...
  return (
    <div ref={ref} aria-hidden="true" style={{ position:'relative', width:320, height:320, flexShrink:0, perspective:'1000px' }}>
...
export function Footer({ reduced }: { reduced: boolean }) {
  return (
    <footer data-seal-section="7" aria-label="Footer" style={{
      ...
      <div style={{
        position:'relative',
        display:'grid', gridTemplateColumns:'minmax(440px,1.05fr) .95fr',
        alignItems:'center', gap:'clamp(2rem,4vw,4rem)',
        padding:'clamp(2.5rem,5vh,4rem) clamp(2rem,5vw,5rem) clamp(2rem,4vh,3rem)',
        minHeight:'660px',
      }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', alignItems:'center' }}>
          <div id="footer-seal-slot" data-seal-anchor="7" role="button" tabIndex={0}
            ...
            onKeyDown={e => { if (e.key === 'Enter') { const r = e.currentTarget.getBoundingClientRect();
              window.dispatchEvent(new CustomEvent('dh-kill', { detail: { x: r.left + r.width / 2, y: r.top + r.height / 2 } })); } }}
            title="歸 · return to the beginning"
            style={{ width:'clamp(420px, 40vw, 640px)', height:'clamp(420px, 40vw, 640px)', position:'relative',
